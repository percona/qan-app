...
export const metricCatalogue = {
  bytes_sent: {
    ...
    metricRelation: data => {
      return `${transform(data[this.simpleName].stats.sum / data['rows_sent'].stats.sum)} per row sent`
    },
    units: Units.PER_SEC,
    pipeTypes: {
      ratePipe: 'size',
  ...
  filesort: {
    ...
    simpleName: 'filesort',
    metricRelation: data => {
      return `${transform(data[this.simpleName].stats.sum / data['num_queries'].stats.sum)} per query`
    },
    units: Units.NONE,
    pipeTypes: {
      ratePipe: 'number',
  ...
  innodb_io_r_bytes: {
    ...
    metricRelation: data => {
      return `${transform(data[this.simpleName].stats.sum / data['innodb_io_r_ops'].stats.sum)} per Read Ops`
    },
    units: Units.PER_SEC,
    pipeTypes: {
      ratePipe: 'size',
  ...
  innodb_io_r_wait: {
    ...
    metricRelation: data => {
      return `${transform(data[this.simpleName].stats.avg / data['query_time'].stats.avg)} of query time`
    },
    units: Units.NONE,
    pipeTypes: {
      ratePipe: 'number',
  ...
  innodb_queue_wait: {
    ...
    tooltipText: 'Shows how long( in seconds) the query spent either waiting to' +
  ...
  merge_passes: {
    ...
    metricRelation: data => {
      return `${transform(data[this.simpleName].stats.sum / data['filesort'].stats.sum)} per external sort`
    },
    units: Units.NONE,
    pipeTypes: {
      ratePipe: 'number',
  ...
  tmp_disk_tables: {
    ...
    metricRelation: data => {
      return `${transform(data[this.simpleName].stats.sum / data['tmp_table_on_disk'].stats.sum)} per query`
    },
    units: Units.NONE,
    pipeTypes: {
      ratePipe: 'number',
  ...
  tmp_tables: {
    ...
    metricRelation: data => {
      return `${transform(data[this.simpleName].stats.sum / data['tmp_table'].stats.sum)} per query`
    },
    units: Units.NONE,
    pipeTypes: {
      ratePipe: 'number',
